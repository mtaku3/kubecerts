...
func (cm *CertManager) getExpectedConfigForCert(h host.Host, certFile string) *cert.CertConfig {
	...
	case "scheduler.crt":
		return cert.NewSchedulerServerConfig(h.Name, h.AdvertiseIP)
...
func (cm *CertManager) getCAFileForCert(certFile string) string {
	switch certFile {
	case "apiserver.crt", "kubelet.crt", "controller-manager.crt", "scheduler.crt", "apiserver-kubelet-client.crt", "kubelet-client.crt", "kube-proxy-client.crt", "controller-manager-client.crt", "scheduler-client.crt":
		return "ca.crt"
	case "apiserver-etcd-client.crt":
		return "etcd/ca.crt"
	case "front-proxy-client.crt":
